...
def drunk(eyes, roi_color, roi_gray):
    for (ex, ey, ew, eh) in eyes:
        ...
        # Convert red_eye to grayscale
        red_eye_gray = cv2.cvtColor(red_eye, cv2.COLOR_BGR2GRAY)

        # Use red_eye_gray with cv2.countNonZero
        if cv2.countNonZero(red_eye_gray) > (0.3 * ew * eh):
            return True
    return False
...
while True:
    ...
    for face in faces:
        ...
        is_sober_eyes = drunk(eyes, roi_color, roi_gray)
